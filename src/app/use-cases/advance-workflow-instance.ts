...
import { WorkflowInstanceRepository } from '../contracts/workflow-instance-repository.js'
...
export class AdvanceWorkflowInstanceUseCase {
  ...
  async execute(input: AdvanceWorkflowInstanceInput): Promise<void> {
    const instance = await this.workflowInstanceRepository.findById(input.instanceId)
